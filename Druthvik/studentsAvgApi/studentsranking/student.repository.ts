...
export class SudentRepository {
  ...
  constructor(private readonly connection: Connection, private readonly rankAllotment: RankAllotment) {}
  ...
  public getAllSTudents  = async () :Promise<Array<Student>> => {
    ...
    stud = await query.select(['student', 'subject', 'mark'])
    .leftJoinAndSelect("student.subject", "subject").innerJoinAndSelect('subject.mark', 'mark')
    .getMany()
    return stud;
  }
